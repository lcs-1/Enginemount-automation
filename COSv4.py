...
def save_document(event=None):
    ...
    if pdf_path:
        ...
        for i, img_path in enumerate(images):
            ...
            if i == 0:
                # Find the placeholder {{Image}} and insert the image there
                for paragraph in doc.paragraphs:
                    if "{{Image}}" in paragraph.text:
                        run = paragraph.add_run()
                        run.add_picture(img_path, width=Inches(6.5))  # Adjust width as needed
                ...
                doc.add_picture(img_path, width=Inches(6.5))  # Adjust width as needed
                print("Image inserted into document:", img_path)  # Debug print
                os.remove(img_path)  # Remove the temporary image file
